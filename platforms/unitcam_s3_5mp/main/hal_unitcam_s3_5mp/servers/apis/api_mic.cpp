...
static void _mic_recording_task(void* param)
{
    ...
    while (1) {
        ...
        while (__get_mic()->isRecording()) vTaskDelay(1);
        rec_psram_index += record_length;
...
void startMicRecording(AsyncWebServerRequest* request)
{
    ...
    if (!is_recording) xTaskCreatePinnedToCore(_mic_recording_task, "mic", 4000, NULL, 3, NULL, 1);
...
void getLastRecord(AsyncWebServerRequest* request)
{
    request->send(LittleFS, "/wav/rec.wav");
}
